...
(function($, window, document, undefined) {
  ...
  var Slider = {
    ...
    addNavigation: function(navClass) {
      var self = this;
      var cancelBT =
        '<a class="crf_btns" alt="Cancel" title="Cancel" id="crf_cancel" onclick="$N.listCRFs();"></a>';
      var printBT = '<a class="crf_btns" alt="Print" title="Print" id="crf_print" onclick="$N.printCRF();"></a>';
      var submitBT = $N.hasPermission('edit')?
        '<a class="crf_btns" alt="Submit CRF" title="Submit CRF" id="crf_submit" onclick="$N.submitCRF();"></a>':'';
      var logBT = '<a class="crf_btns" alt="CRF LOG" title="CRF LOG" id="crf_log" onclick="$N.modals.log();"></a>';
      var diffBT = '<a class="crf_btns" alt="DIFF LOG" title="DIFF LOG" id="crf_diff" onclick="$N.modals.diff();"></a>';
      var dynamicTabsElm = '<' + self.options.navElementTag + ' class="ls-nav"><ul id="' +
        (self.$elem).attr('id') + '-nav-ul"></ul></' + self.options.navElementTag + '>';
      // Navigation buttons
      if(N.study.doubles && N.current.form!='master'){ (self.$sliderId).before(submitBT+logBT+cancelBT+diffBT+printBT+dynamicTabsElm) }
      else{ (self.$sliderId).before(submitBT+logBT+cancelBT+printBT+dynamicTabsElm) }
      // Buttons in the bottom
      // (self.$sliderId).after(dynamicTabsElm+submitBT+logBT+cancelBT) }
